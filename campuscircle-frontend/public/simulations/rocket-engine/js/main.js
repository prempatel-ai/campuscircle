...
(function (global) {
  ...
  /* The rectangle of screen not covered by panels, where the world should sit. */
  ...
  /* The default view rides with the carrier rather than showing the whole
     site, because the tour is about what the carrier is having done to it.
     Zooming out to the whole plant is one click (or a double-click). */
  function defaultScale() { return viewW <= 900 ? 0.85 : 1.15; }

  function fitFactory() {
    /* the site is the slab plus the pad below it, so fit the taller box */
    var w = (F.GW + F.PAD.y + F.PAD.r) * Iso.TW;
    var h = (F.GW + F.PAD.y + F.PAD.r) * Iso.TH + 220;
    var r = availableRect();
    cam.scale = clamp(Math.min((r.w - 32) / w, (r.h - 32) / h), 0.10, 1.4);
    var c = Iso.project(F.GW / 2, (F.GH + F.PAD.y) / 2, 0);
    cam.x = c.x; cam.y = c.y;
  }
  ...
  function zoomAt(mx, my, factor) {
    ...
    /* the low end has to reach far enough for the whole site to fit a phone */
    cam.scale = clamp(cam.scale * factor, 0.10, 2.4);
    var f = focusPoint();
  ...
  function frame(now) {
    ...
    } else if (Sim.state.launch.active) {
      /* The launch overrides Follow: the whole point of the last thirty
         seconds is up in the air, and leaving the camera on the empty pad
         because a checkbox was unticked would be a bad joke. */
      var lp = cameraTargetPoint();
      cam.x += (lp.x - cam.x) * k;
      cam.y += (lp.y - cam.y) * k;
      /* pull back so the whole vehicle stays in shot, and keep pulling back as
         it climbs away */
      var want = defaultScale() * (0.56 - 0.22 * Sim.launchProgress());
      cam.scale += (want - cam.scale) * k;
    } else if (follow) {
      var cp = cameraTargetPoint();
      cam.x += (cp.x - cam.x) * k;
      cam.y += (cp.y - cam.y) * k;
    }
